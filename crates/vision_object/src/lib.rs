...
let (shape_usize, data) = {
    ...
    fn parse_yolo_output(&self, output: ArrayViewD<f32>, img_w: f32, img_h: f32) -> Vec<ObjectDetection> {
...
detections.push(ObjectDetection {
    ...
    fn apply_nms(&self, mut detections: Vec<ObjectDetection>, iou_threshold: f32) -> Vec<ObjectDetection> {
        ...
        while !detections.is_empty() {
            ...
            detections.retain(|item| {
                intersection_over_union(&best.box_2d, &item.box_2d) < iou_threshold
            });
